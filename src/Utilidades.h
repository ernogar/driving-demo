...
void texto(unsigned int x, unsigned int y, char *text, const GLfloat *color = ROJO, void *font = GLUT_BITMAP_HELVETICA_18);
/* Muestra en la posicion (x, y) la cadena de texto con la fuente y el color indicados
   Los dos ultimos parametros son opcionales. Ver en glut.h las posibles fuentes que se 
   pueden utilizar. 
   ...
   Las normales se generan como (0,0,1)            */
{
	...
	static const float delta = 1.0/resolucion;		//distancia entre dos consecutivos
	// generacion de coordenadas
	GLfloat* vertices = new GLfloat[NP*NP*3];
	int cont = 0;
	for(int i = 0; i < NP; i++)
		for(int j = 0; j < NP; j++){
			vertices[cont++] = -0.5 + delta*j;		//x
			vertices[cont++] = -0.5 + delta*i;		//y
			vertices[cont++] =  0.0;				//z
		}
...
// Dibuja un cuadrilatero con resolucion MxN y fija la normal 
{
	...
	for(int i=0; i<M; i++){
		...
		for(int j=0; j<=N; j++){
			...
			// punto izquierdo
			glTexCoord2f(i*1.0/M, j*1.0/N);  // s,t
...
void texto(unsigned int x, unsigned int y, char *text, const GLfloat *color, void *font)
{	
	int viewport[4];
